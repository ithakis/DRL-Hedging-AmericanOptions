...
class TradingEnv(gym.Env):
    ...
    def __init__(
        self, utils, logger: Optional[loggers.Logger] = None, load_data_from_file=False, portfolio_folder=None
    ):
        ...
        self.portfolio = MainPortfolio(
            utils, load_data_from_file=load_data_from_file, portfolio_folder=portfolio_folder
        )
        self.utils = utils
        ...
        obs_lowbound = np.array(
            [self.portfolio.a_price.min(), -1 * max_gamma * self.utils.contract_size, -np.inf]
        )
        obs_highbound = np.array([self.portfolio.a_price.max(), max_gamma * self.utils.contract_size, np.inf])
        if FLAGS.vega_obs:
            obs_lowbound = np.concatenate([obs_lowbound, [-1 * max_vega * self.utils.contract_size, -np.inf]])
            obs_highbound = np.concatenate([obs_highbound, [max_vega * self.utils.contract_size, np.inf]])
    ...
    def step(self, action):
        ...
        # Action constraints
        gamma_action_bound = (
            -self.portfolio.get_gamma(self.t)
            / self.portfolio.hed_port.options[self.sim_episode, self.t].gamma_path[self.t]
            / self.utils.contract_size
        )
        ...
        if FLAGS.vega_obs:
            # Vega bounds
            vega_action_bound = (
                -self.portfolio.get_vega(self.t)
                / self.portfolio.hed_port.options[self.sim_episode, self.t].vega_path[self.t]
                / self.utils.contract_size
            )
