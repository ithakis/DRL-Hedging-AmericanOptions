...
def validate_input(func):
    ...
    def wrapper(self, *args, **kwargs):
        ...
        validations = {
            ...
            'q': (
                lambda x: isinstance(x, (int, float)),
                "q must be a float representing the dividend yield."
            ),
            'spread': (
                lambda x: isinstance(x, (int, float)) and x >= 0,
            ...
            'num_conts_to_add': (
                ...
                "num_conts_to_add must be an integer."
            ),
            'contract_size': (
                lambda x: isinstance(x, int) and x > 0,
            ...
            'action_low': (
                lambda x: isinstance(x, (int, float)),
                "action_low must be a numerical value."
            ),
            'action_high': (
                lambda x: isinstance(x, (int, float)),
                "action_high must be a numerical value."
            ),
        }
...
class Utils:
    ...
    def __init__(self, 
        ...
        S0: float, K: float, init_ttm: float = 120, r: float = 0.0, q: float = 0.0, spread: float = 0.0,
        # Properties of the Simulation for the Liability Portfolio
        ttms: List[int] = [120], poisson_rate: float = 1.0, TradingDaysPerYear: int = 252, moneyness_mean: float = 1.0, moneyness_std: float = 0.0,
        
        # Hedging Portfolio Parameters
        hed_ttm: float = 60, hed_type: str = 'European',

        # init_vol is for both GBM and Heston
        init_vol: float = None,
        # Heston, Model Parameters
        kappa: float = None, theta: float = None, volvol: float = None, rho: float = None,
        
        # Simulation Parameters
        stochastic_process: str = None, time_to_simulate: float = None, num_sim: int = None, 
        frq: int = 1, numerical_accuracy: str = None, n_jobs: int = None, np_seed: int = 1234,

        # Contract Parameters
        num_conts_to_add: int = -1, contract_size: int = 100,
    ...
    def get_sim_path_GBM(self):
        ...
        for t in range(self.num_period):
            a_price[:, t + 1] = a_price[:, t] * np.exp(
                (self.r - self.q - (self.init_vol ** 2) / 2) * self.dt + self.init_vol * np.sqrt(self.dt) * z[:, t]
            )
    ...
    def get_sim_path_Heston(self, psiC=1.5, gamma1=0.5, gamma2=0.5, Martingale_Correction=True, _show_progress=True):
        ...
        K1 = (kappa * rho / xi - 1 / 2) * gamma1 * dt - rho / xi
        K2 = (kappa * rho / xi - 1 / 2) * gamma2 * dt + rho / xi
        K3 = (1 - rho ** 2) * gamma1 * dt
        K4 = (1 - rho ** 2) * gamma2 * dt 
        A = K2 + 0.5 * K4
        if Martingale_Correction: K0_star = np.empty(N_paths)

        sobol_gen = qmc.Sobol(d=T_steps + 1, seed=self.seed)
    ...
    def _American_Option_GBM(S0:float, K:float, T:float, r:float, q:float, sigma:float, N=1000, option_type='put'):
        """
    ...
    @staticmethod
    def _American_Option_Heston(S0, K, T, r, q, v0, kappa, theta, volvol, rho, N_gridpoints=100, Type='put', exercise='American'):
        """
        ...
        """
        if 2*kappa*theta < volvol**2: print(f"Warrning: Feller condition violated: 2*kappa*theta < volvol**2")
        ...
        exercise = ql.AmericanExercise(today, exerciseDate) if exercise == 'American' else ql.EuropeanExercise(exerciseDate)
    ...
    def American_put_option(self, iv: np.ndarray, ttms: np.ndarray, S0: np.ndarray, K: np.ndarray, T:int=252, stochastic_process:str=None):
        """
        ...
        if stochastic_process == 'GBM':
            ...
            def calculate_gbm_option(idx):
                ...
                C0, delta, gamma = self._American_Option_GBM(
                    S0=S0[idx], K=K[idx], T=fT, r=self.r, q=self.q, sigma=iv[idx],
                    option_type='put', N=N
                )
                C0_up = self._American_Option_GBM(
                    S0=S0[idx], K=K[idx], T=fT, r=self.r, q=self.q, sigma=iv[idx]*(1+eps_iv),
                    option_type='put', N=N
                )[0]
            ...
            def calculate_heston_option(idx):
                ...
                C0, delta, gamma = self._American_Option_Heston(
                    S0=S0[idx], K=K[idx], T=fT, r=self.r, q=self.q, v0=v0, 
                    theta=self.theta, kappa=self.kappa, volvol=self.volvol, rho=self.rho, 
                    N_gridpoints=N_gridpoints, Type='put', exercise='American'
                )

                C0_iv_up = self._American_Option_Heston(
                    S0=S0[idx], K=K[idx], T=fT, r=self.r, q=self.q, v0=v0_up,
                    theta=theta_up, kappa=self.kappa, volvol=self.volvol, rho=self.rho,
                    N_gridpoints=N_gridpoints, Type='put', exercise='American'
                )[0]
            ...
            raise ValueError(f"Stochastic process must be either 'GBM' or 'Heston' but got {stochastic_process}")

        return price, delta, gamma, vega


    @staticmethod
    def _European_Option_GBM(iv, ttm: np.ndarray, S: np.ndarray, K: np.ndarray, r:float, q:float, T:int=252):
        """
    ...
    @staticmethod
    def _European_Option_Heston(S0, K, T, r, q, v0, kappa, theta, volvol, rho, Type='put', accuracy=1e-9, max_evaluations=1_000_000):
        """
        ...
        while True:
            ...
            try:
                ...
                if "max number of iterations reached" in str(e): accuracy *= 10
                else:   raise


    def European_put_option(self, iv: np.ndarray, ttms: np.ndarray, S0: np.ndarray, K: np.ndarray, T:int=252, stochastic_process:str=None):
        """
        ...
        if stochastic_process == 'GBM':
            ...
            def calculate_heston_option(idx):
                ...
                C0 = self._European_Option_Heston(
                    S0=S0[idx], K=K[idx], T=fT, r=self.r, q=self.q, v0=v0, kappa=self.kappa, theta=self.theta,
                    volvol=self.volvol, rho=self.rho, Type=opt_type, accuracy=accuracy, max_evaluations=max_evaluations
                )

                # Perturb S0 for Delta and Gamma
                C0_S_up = self._European_Option_Heston(
                    S0=S0[idx] * (1 + eps_S), K=K[idx], T=fT, r=self.r, q=self.q, v0=v0, kappa=self.kappa,
                    theta=self.theta, volvol=self.volvol, rho=self.rho, Type=opt_type, accuracy=accuracy, max_evaluations=max_evaluations
                )
                ...
                C0_iv_up = self._European_Option_Heston(
                    S0=S0[idx], K=K[idx], T=fT, r=self.r, q=self.q, v0=v0_up, kappa=self.kappa, theta=theta_up,
                    volvol=self.volvol, rho=self.rho, Type=opt_type, accuracy=accuracy, max_evaluations=max_evaluations
                )
                C0_iv_down = self._European_Option_Heston(
                    S0=S0[idx], K=K[idx], T=fT, r=self.r, q=self.q, v0=v0_down, kappa=self.kappa, theta=theta_down,
                    volvol=self.volvol, rho=self.rho, Type=opt_type, accuracy=accuracy, max_evaluations=max_evaluations
                )
    ...
    def agg_poisson_dist(self, a_prices, vol):
        ...
        for step_i in tqdm(range(num_step)):
            ...
            option_ttms = np.random.choice(self.ttms, (num_episode, step_num_opts)) # Default value for self.ttms is [120]
            ...
            step_port_price, step_port_delta, step_port_gamma, step_port_vega = \
                self.American_put_option(iv=step_vol, ttms=all_option_ttms, S0=step_a_prices, K=all_option_strikes, T=self.TradingDaysPerYear, stochastic_process=self.stochastic_process)
        ...
        for ep_i in tqdm(range(num_episode)):
            options.append(
                SyntheticOption(port_price[ep_i,:], next_port_price[ep_i,:], port_delta[ep_i,:], port_gamma[ep_i,:], port_vega[ep_i,:], 
                                np.zeros((num_step,)).astype(np.bool_), self.num_conts_to_add, self.contract_size))
        ...
        **Usage:**
        ...
        for step_i in tqdm(range(num_step)):
            ...
            if self.hed_type == 'European':
                step_option_price, step_option_delta, step_option_gamma, step_option_vega = \
                    self.European_put_option(iv=step_vol, ttms=all_option_ttms, S0=step_a_prices, K=all_option_strikes, T=self.TradingDaysPerYear, stochastic_process=self.stochastic_process)
