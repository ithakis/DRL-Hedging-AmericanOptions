...
class Option(AssetInterface):
    ...
    def __init__(
        self, price_path, delta_path, gamma_path, vega_path, inactive, num_contract, contract_size=100
    ):
        """Constructor
...
class SyntheticOption(Option):
    ...
    def __init__(
        ...
        """
        super().__init__(
            cur_price_path, delta_path, gamma_path, vega_path, inactive, num_contract, contract_size
        )
    ...
    def __repr__(self):
        ...
        return f"{base_repr[:-1]}, next_price_path_shape={self.next_price_path.shape})"
        ...
        if load_data_from_file:
            ...
            if portfolio_folder is not None:
                ...
                with open(full_folder_path / "Utils parameters.txt", "w") as txt_file:
                    ...
                    txt_file.write(
                        f"S0={utils_params['S0']}, K={utils_params['K']}, ttms={utils_params['ttms']}, "
                    ...
                    txt_file.write(
                        f"hed_ttm={utils_params['hed_ttm']}, hed_type='{utils_params['hed_type']}',\n"
                    )
                    txt_file.write("\n# init_vol is for both GBM and Heston\n")
                    txt_file.write(f"init_vol={utils_params['init_vol']},\n")
                    txt_file.write("# Heston, Model Parameters\n")
                    txt_file.write(
                        f"kappa={utils_params['kappa']}, theta={utils_params['theta']}, "
                        f"volvol={utils_params['volvol']}, rho={utils_params['rho']},\n"
                    )
                    txt_file.write("\n# Simulation Parameters\n")
                    txt_file.write(
                        f"stochastic_process='{utils_params['stochastic_process']}', time_to_simulate={utils_params['time_to_simulate']}, "
                        f"num_sim={utils_params['num_sim']}, frq={utils_params['frq']},\n"
                    )
                    txt_file.write(
                        f"numerical_accuracy='{utils_params['numerical_accuracy']}', n_jobs={utils_params['n_jobs']}, np_seed={utils_params['np_seed']},\n"
                    )
                    txt_file.write("\n# RL Environment Parameters\n")
                    txt_file.write(
                        f"action_low={utils_params['action_low']}, action_high={utils_params['action_high']},\n"
                    )
