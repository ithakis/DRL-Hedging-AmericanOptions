...
flags.DEFINE_float('mu',                0.0,        'Equity Risk Premium (Default 0.0)')
...
def main(argv):
    ...
    if FLAGS.portfolio_folder:
        ...
        if not main_portfolio_folder.exists():
            if FLAGS.eval_only:
                # Create the portfolio folder structure if we're in eval_only mode
                print(f"Creating new portfolio folder structure at '{main_portfolio_folder}'")
                main_portfolio_folder.mkdir(parents=True, exist_ok=True)
                (main_portfolio_folder / 'Train').mkdir(exist_ok=True)
                (main_portfolio_folder / 'Eval').mkdir(exist_ok=True)
            else:
                # For training mode, still require the folder to exist
    ...
    utils = Utils(
        ...
        S0=FLAGS.S0, K=FLAGS.K, init_ttm=FLAGS.init_ttm, r=FLAGS.r, mu=FLAGS.mu, q=0.00, spread=FLAGS.spread,
    ...
    if FLAGS.critic == 'c51':
        ...
        agent_networks = make_quantile_networks(action_spec=eval_environment_spec.actions)
